...
<head>
    ...
    <title>open_checklist_note</title>
    <base href="<?= $web_root ?>">
    <!-- Pour bootsrap -->
...
<body>
    <header class="container-fluid d-flex justify-content-between mt-3">
        ...
        <div>
            <?php if($estPartagee) : ?>
                <?php if($maNote instanceof CheckListNotes) : ?>
        ...
        <?php if(!$estPartagee) : ?>
        <div class="d-flex justify-content-between headerRight">
            <!-- Epingler ou désépingler la note -->
            <a href="Notes/pinned_note/<?= $id; ?>">
            <?php if(isset($newPinnedStatus)) : ?>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-pin-fill mr-2" viewBox="0 0 16 16">
            ...
            <!--  Archiver la note -->
            <a href="Notes/archive_text_note/<?= $id; ?>">
            ...
            <!--  Label  -->
            <a href="Notes/labels/<?= $id; ?>">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-tag" viewBox="0 0 16 16">
                <path d="M6 4.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0m-1 0a.5.5 0 1 0-1 0 .5.5 0 0 0 1 0"/>
                <path d="M2 1h4.586a1 1 0 0 1 .707.293l7 7a1 1 0 0 1 0 1.414l-4.586 4.586a1 1 0 0 1-1.414 0l-7-7A1 1 0 0 1 1 6.586V2a1 1 0 0 1 1-1m0 5.586 7 7L13.586 9l-7-7H2z"/>
            </svg>
            </a>
            <!-- Editer la note la note -->
            <a href="Notes/view_edit_checklist_note/<?= $id ?>">
    ...
    <div class="main-container container-fluid">
        <p class="italique mt-2">Created <?= $maNote->calculDateCreated()?>. <?=  $maNote->calculDateEdited()?></p>
        <form action="Notes/open_checklist/<?= $id ?>" method="post" id="formCheck">
            <!-- ------------ Titre ---------------- -->
            <label for="title">Title</label>
            <div class="input-group mb-3">
                    <input type="text" class="form-control gris borderNone" id="title" value="<?= $maNote->title ?>" readonly>
            </div>
            <!-- --------- Items------------ -->
            <div class="input-group "> 
                <label class="mt-2">Items</label>
                <!-- Pour chaque case à cocher -->
                <?php foreach($maNote->checkListItems as $item): ?>
                    <div class="input-group mb-3 bg-primary borderNone">
                        <button type="submit" name="item_id" value="<?= $item->id ?>" class="btn btn-primary borderRadius">
                            <?php if($item->checked) :?>
                                <!-- SVG pour le cas où l'élément est coché -->
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-square-fill" viewBox="0 0 16 16">
                                    <path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zm10.03 4.97a.75.75 0 0 1 .011 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.75.75 0 0 1 1.08-.022z"/>
                                </svg>
                                    <?php else : ?> 
                                <!-- SVG pour le cas où l'élément n'est pas coché -->
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-square" viewBox="0 0 16 16">
                                    <path d="M14 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zM2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z"/>
                                </svg>
                            <?php endif; ?>
                        </button>
                        <input type="text" class="form-control borderNone gris <?= $item->checked ? 'checked-title' : '' ?>" name="title" value="<?= $item->content ?>"  readonly>
                    </div>
                <?php endforeach; ?>
            </div>
        </form>
